...
class Model_reseller extends CI_model{
    ...
    function profile_konsumen($id){
        return $this->db->query("SELECT a.id_konsumen, a.username, a.nama_lengkap, a.email, a.jenis_kelamin, a.tanggal_lahir, a.tempat_lahir, a.alamat_lengkap, a.kecamatan_id, a.no_hp, a.tanggal_daftar, b.kota_id, b.nama_kota as kota, c.provinsi_id, c.nama_provinsi as propinsi, a.foto FROM `rb_konsumen` a LEFT JOIN rb_kota b ON a.kota_id=b.kota_id LEFT JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id where a.id_konsumen='$id'");
    }
        ...
        if (trim($this->input->post('a')) != ''){
            if ($hasil['file_name']==''){
                $datadbd = array('username'=>$this->db->escape_str(strip_tags($this->input->post('aa'))),
                            ...
                            'provinsi_id'=>$this->db->escape_str(strip_tags($this->input->post('provinsi_id'))),
                            'kecamatan_id'=>$this->db->escape_str(strip_tags($this->input->post('kecamatan_id'))),
    ...
    function reseller_pembelian($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_reseller b ON a.id_pembeli=b.id_reseller where a.status_penjual='$level' AND a.id_pembeli='$id' AND a.service!='Stok Otomatis (Pribadi)' ORDER BY a.id_penjualan DESC");
    }
    ...
    function penjualan_list($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_reseller b ON a.id_pembeli=b.id_reseller where a.status_penjual='$level' AND a.id_penjual='$id' AND service!='Stok Otomatis (Pribadi)' ORDER BY a.id_penjualan DESC");
    }
    ...
    function penjualan_perusahaan($id_reseller){
        return $this->db->query("SELECT sum((a.harga_jual-a.diskon)*a.jumlah) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_produk=b.id_produk JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_penjual='reseller' AND id_penjual='".$id_reseller."' AND c.proses>'3'");
    }
    ...
    function produk_perkategori($id_reseller,$id_produk_perusahaan,$id_kategori_produk,$limit){
        return $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id 
                                        ...
                                        where a.id_reseller!='$id_reseller' AND a.id_produk_perusahaan='$id_produk_perusahaan' AND a.id_kategori_produk='$id_kategori_produk' AND aktif='Y' ORDER BY RAND() DESC LIMIT $limit");
    }
    ...
    function produk_diskon($id_reseller,$id_produk_perusahaan,$limit){
        return $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id 
                                        LEFT JOIN rb_produk_diskon d ON a.id_produk=d.id_produk
                                        where a.id_reseller!='$id_reseller' AND a.id_produk_perusahaan='$id_produk_perusahaan' AND aktif='Y' AND d.diskon>'0' ORDER BY RAND() LIMIT $limit");
    }
    ...
    function produk_flashdeal($id_reseller,$id_produk_perusahaan,$limit){
        return $this->db->query("SELECT z.id_produk_penawaran, a.*, b.nama_reseller, c.nama_kota FROM rb_produk_penawaran z JOIN rb_produk a ON z.id_produk=a.id_produk LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id 
                                        where a.id_reseller!='$id_reseller' AND a.id_produk_perusahaan='$id_produk_perusahaan' AND aktif='Y' ORDER BY RAND() LIMIT $limit");
    }
    ...
    function produk_terbaru($id_reseller,$id_produk_perusahaan,$limit){
        return $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id 
                                        LEFT JOIN rb_produk_diskon d ON a.id_produk=d.id_produk
                                        where a.id_reseller!='$id_reseller' AND a.id_produk_perusahaan='$id_produk_perusahaan' AND aktif='Y' ORDER BY a.waktu_input DESC LIMIT $limit");
    }
    ...
    function detail_produk_terbaru($id_reseller,$id_produk_perusahaan,$id_kategori_produk,$limit){
        return $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id 
                                        LEFT JOIN rb_produk_diskon d ON a.id_produk=d.id_produk
                                        where a.id_reseller!='$id_reseller' AND a.id_produk_perusahaan='$id_produk_perusahaan' AND a.id_kategori_produk='$id_kategori_produk' AND aktif='Y' ORDER BY a.waktu_input DESC LIMIT $limit");
    }

    function produk_terjual($id_produk,$proses){
        return $this->db->query("SELECT * FROM `rb_penjualan_detail` a JOIN rb_penjualan b ON a.id_penjualan=b.id_penjualan where a.id_produk='$id_produk' AND b.proses>$proses");
    }

    function cari_produk($kata,$filter){
            $pisah_kata = explode(" ",$kata);
            $jml_katakan = (integer)count($pisah_kata);
            $jml_kata = $jml_katakan-1;
            if (cetak($filter)=='0'){
                $cari = "SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_reseller!='0' AND a.id_produk_perusahaan='0' AND a.aktif='Y' AND";
                    for ($i=0; $i<=$jml_kata; $i++){
                ...
                $cari .= " ORDER BY a.id_produk DESC LIMIT 24";
            }else{
                $exp = explode('|',cetak($filter));
				if ($exp[0]=='kategori'){
                    $cari = "SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_reseller!='0' AND a.id_produk_perusahaan='0' AND a.aktif='Y' AND a.id_kategori_produk='".$exp[1]."' AND";
                    for ($i=0; $i<=$jml_kata; $i++){
                    $cari .= " a.nama_produk LIKE '%".$pisah_kata[$i]."%'";
                        if ($i < $jml_kata ){
                            $cari .= " OR "; 
                    ...
                    $cari = "SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_reseller!='0' AND a.id_produk_perusahaan='0' AND a.aktif='Y' AND a.id_kategori_produk_sub='".$exp[1]."' AND";
                    for ($i=0; $i<=$jml_kata; $i++){
                    $cari .= " a.nama_produk LIKE '%".$pisah_kata[$i]."%'";
                        if ($i < $jml_kata ){
                            $cari .= " OR "; 

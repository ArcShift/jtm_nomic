			...
			if (cetak($this->input->post('password'))==cetak($this->input->post('repassword'))){
				...
				if ($cek_username->num_rows()<='0'){
					...
					$message  = "<html><body><img src='".base_url()."asset/logo/$logo[gambar]' style='height:87px'><br>
											<span style='font-size:18px; color:green'>Selamat Bergabung di $iden[url]</span><br>
		...
		if($this->facebook->is_authenticated()){
			...
			if ($rows['id_konsumen']!=''){
    			...
    			if ($this->session->id_konsumen!=''){
    			    ...
    			    echo $this->session->set_flashdata('message', '<div class="alert alert-danger"><b>GAGAL</b> -  Akun anda tidak ditemukan di Database Kami...</div>');
    				$iden = $this->db->query("SELECT * FROM identitas where id_identitas='1'")->row_array();
    				$data['title'] = "Masuk / Daftar Di $iden[url]";

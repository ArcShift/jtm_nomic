...
class Administrator extends CI_Controller {
    ...
    function withdraw(){
        ...
        $data['record'] = $this->db->query("SELECT * FROM rb_withdraw a JOIN rb_reseller b ON a.id_reseller=b.id_reseller JOIN rb_rekening_reseller c ON a.id_rekening_reseller=c.id_rekening_reseller ORDER BY a.waktu_withdraw DESC");
    ...
    function pembayaran_konsumen(){
        ...
        $data['record'] = $this->db->query("SELECT a.*, b.*, c.kode_transaksi, c.proses FROM `rb_konfirmasi_pembayaran_konsumen` a JOIN rb_rekening b ON a.id_rekening=b.id_rekening JOIN rb_penjualan c ON a.kode_transaksi=c.kode_transaksi where c.status_penjual='reseller' GROUP BY c.kode_transaksi");
    ...
    function mutasi_ty35fgdfgd777bba064b72be(){
		...
		foreach ($data['data_mutasi'] as $dtm) { //DATA MUTASI
			...
			if ($type=='CR'){
			$unik = $this->db->query("SELECT a.*, b.proses, SUBSTR(timediff(now(), a.waktu_proses),1,2) as durasi FROM `rb_penjualan_otomatis` a JOIN rb_penjualan b ON a.kode_transaksi=b.kode_transaksi where a.nominal='$amount' AND b.proses='0' AND SUBSTR(timediff(now(), a.waktu_proses),1,2)<'24' GROUP BY b.kode_transaksi");
                if ($unik->num_rows()=='1'){
                    $mut = $unik->row_array();
                    ...
                    $subject      = 'TOKOSA.COM - Pendaftaran Sukses,..';
                    ...
                    $penjual = $this->db->query("SELECT b.id_penjual FROM rb_penjualan_otomatis a JOIN rb_penjualan b ON a.kode_transaksi=b.kode_transaksi where b.status_pembeli='konsumen' AND a.kode_transaksi='$mut[kode_transaksi]' GROUP BY b.id_penjual");
                    foreach ($penjual->result_array() as $row){
                        $subject_ress      = 'TOKOSA.COM - Pembayaran dari Pembeli Sukses,..';
	...
	function mutasi(){
		...
		CURLOPT_URL => "https://mutasibank.co.id/api/v1/statements/1172",

...
class Reseller extends CI_Controller {
	function index(){
		if (isset($_POST['submit'])){
			...
			if ($total > 0){
				...
				$data['title'] = 'Reseller &rsaquo; Log In';
	...
	function tambah_produk(){
        ...
        if (isset($_POST['submit'])){
			...
			$cek_produk = $this->db->query("SELECT * FROM rb_produk where id_reseller='".$this->session->id_reseller."'");
			$status = cek_paket($this->session->id_reseller);
			if ($status>=1){
				$cekpa = $this->db->query("SELECT a.id_reseller, b.max_produk, b.nama_paket FROM `rb_reseller_paket` a JOIN rb_paket b ON a.id_paket=b.id_paket where a.status='Y' AND a.id_reseller='".$this->session->id_reseller."'")->row_array();
	...
	function withdraw(){
		cek_session_reseller();
		$data['record'] = $this->db->query("SELECT * FROM rb_withdraw a LEFT JOIN rb_rekening_reseller b ON a.id_rekening_reseller=b.id_rekening_reseller where a.id_reseller='".$this->session->id_reseller."' ORDER BY a.id_withdraw DESC");
		$this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_rekening/view_withdraw',$data);
	}

	function tambah_withdraw(){
		cek_session_reseller();
		if (isset($_POST['submit'])){
			$data = array('id_rekening_reseller'=>$this->input->post('a'),
						  'id_reseller'=>$this->session->id_reseller,
			...
			$this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_rekening/view_withdraw_tambah');
		}
	}
	...
	function penjualan(){
		...
		$data['diterima'] = $this->model_reseller->pembayaran_diterima($id,'reseller');
	...
	function detail_penjualan(){
		...
		if (isset($_POST['submit'])){
				$data1 = array('no_resi'=>$this->input->post('no_resi'),
								'proses'=>'3');
				$where = array('id_penjualan' => $this->uri->segment(3),'id_penjual'=>$this->session->id_reseller);
				$this->model_app->update('rb_penjualan', $data1, $where);
	...
	function upgrade(){
		cek_session_reseller();
		if (isset($_GET['paket'])){
			$rowp = $this->db->query("SELECT * FROM rb_reseller_paket a JOIN rb_paket b ON a.id_paket=b.id_paket where a.id_reseller='".$this->session->id_reseller."'")->row_array();
			if ($rowp['status']=='Y'){
				...
				$cekp = $this->db->query("SELECT * FROM rb_reseller_paket where id_reseller='".$this->session->id_reseller."' AND status='N'");
				...
				if ($cekp->num_rows()<='0'){
					...
					$where = array('id_reseller' => $this->session->id_reseller,'status' => 'N');
			...
			$this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_reseller/paket',$data);
		}
	}
